...
class RNN(nn.Cell):
    ...
    def __init__(self, _input_size, _hidden_size, _num_layers, _num_classes):
        ...
        self.linear = nn.Dense(_hidden_size, _num_classes, weight_init=HeUniform(math.sqrt(5)))

    def construct(self, x):
        # 设置初始状态
        hidden0 = ops.zeros(size=(self.num_layers, x.shape[0], self.hidden_size))
        cell0 = ops.zeros(size=(self.num_layers, x.shape[0], self.hidden_size))
